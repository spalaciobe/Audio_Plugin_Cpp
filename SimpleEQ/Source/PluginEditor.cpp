...
void LookAndFeel::drawToggleButton(juce::Graphics &g, juce::ToggleButton &toggleButton, bool shouldDrawButtonAsHighlighted, bool shouldDrawButtonAsDown)
{
    ...
    if (auto pb = dynamic_cast<PowerButton*>(&toggleButton))
    {
		...
		powerButton.startNewSubPath(r.getCentreX(), r.getY() + 1);
    ...
    else if (auto* analizerButton = dynamic_cast<AnalizerButton*>(&toggleButton))
    {
		auto color = ! toggleButton.getToggleState() ? Colours::dimgrey : Colour(0u, 172u, 1u);
        ...
        g.strokePath(analizerButton->randomPath, PathStrokeType(1.f));
...
{
    if (auto* choiceParam = dynamic_cast<juce::AudioParameterChoice*>(param)) return choiceParam->getCurrentChoiceName();
...
void PathProducer::process(juce::Rectangle<float> fftBounds, double sampleRate)
{
    ...
    */
    ...
    /*
    while there are paths can be pull
...
void ResponseCurveComponent::resized()
{
    ...
    Array<float> xsToWrite;
    for (auto f : freqsToWrite)
    {
        int index = freqs.indexOf(f); 
        if (index != -1) 
        {
            xsToWrite.add(xs[index]); 
        }
    }

    for (int i = 0; i < freqsToWrite.size(); ++i)
    {
        auto f = freqsToWrite[i];
        auto x = xsToWrite[i];
